...
window.onload = function() {
	...
	game.preload( 'chara1.gif','map2.gif','icon0.gif','panel.png','panel2.png',
				  'clear.png','mask.png','effect.gif',
				  'title.png','title_1.png','title_2.png','title_3.png','title_4.png',
	...
	game.onload = function (){
		...
		var level = 1;				//エンドレスモード時ゲームレベル
		var levelup = 30000;		//レベルアップスコア単位
		...
		title.board.image = game.assets['mask.png'];
		title.board.x = 0;
		title.board.y = 0;
		title.addChild(title.board);

		title.t1 = new Sprite(222,42);
		title.t1.image = game.assets['title_1.png'];
		title.t1.x = 10;
		title.t1.y = 40;
//		title.t1.rotation = -10;
		title.t1.opacity = 1;
		title.t1.scaleX = title.t1.scaleY = 4;
		title.t1.tl.scaleTo(1,1,sec(1),enchant.Easing.BOUNCE_EASEOUT).delay(sec(1)).
		rotateTo(-15,sec(0.5),enchant.Easing.BOUNCE_EASEOUT).delay(sec(1)).
		rotateTo(0,sec(0.5),enchant.Easing.BOUNCE_EASEOUT);
		title.addChild(title.t1);

		title.t2 = new Sprite(214,42);
		title.t2.image = game.assets['title_2.png'];
		title.t2.x = 100;
		title.t2.y = 120;
//		title.t2.rotation = -10;
		title.t2.opacity = 0;
		title.t2.scaleX = title.t2.scaleY = 4;
		title.t2.tl.delay(sec(0.5)).show().scaleTo(1,1,sec(1),enchant.Easing.BOUNCE_EASEOUT);
		title.addChild(title.t2);

		title.t3 = new Sprite(207,33);
		title.t3.image = game.assets['title_3.png'];
		title.t3.x = -214;
		title.t3.y = 200;
		title.t3.opacity = 1;
		title.t3.tl.show().moveTo(160-214/2,200,sec(1),enchant.Easing.QUINT_EASEOUT);
		title.addChild(title.t3);

		title.t4 = new Sprite(203,33);
		title.t4.image = game.assets['title_4.png'];
		title.t4.x = 320;
		title.t4.y = 260;
		title.t4.opacity = 1;
		title.t4.tl.show().moveTo(160-203/2,260,sec(1),enchant.Easing.QUINT_EASEOUT);
		title.addChild(title.t4);
		
		title.t5 = new Sprite(16,16);
		title.t5.image = game.assets['icon0.gif'];
		title.t5.frame = 30;
		title.t5.x = 50;
		title.t5.y = -30;
		title.t5.opacity = 1;
		title.t5.scaleX = title.t5.scaleY = 1.5;
//		title.t5.tl.delay(sec(1.5)).moveTo(50,24,sec(0.5),enchant.Easing.QUINT_EASEIN).
		title.t5.vx = 0;
		title.t5.yPrev = -30;
		title.t5.F = 3;
		title.t5.bound = 0;
		title.t5.power = 5;	//バウンド係数
		title.t5.onenterframe = function(){
			if( this.age < sec(1.5) || this.bound == 3 )return;
			var yTemp = this.y;
		...
		moveTo(160,90,sec(0.5),enchant.Easing.QUINT_EASEIN);
		title.addChild(title.t5);


		title.onenter = function(){
		}
		title.addEventListener('touchend', function(e){
			if( e.x > 53 && e.y > 200 && e.x < 260 && e.y < 233 ){
				endless = false;
				stage.initStage();
				game.popScene();
			}
			if( e.x > 59 && e.y > 266 && e.x < 262 && e.y < 293 ){
				endless = true;
				stage.num = 0;
				life.dec();
				life.dec();
		...
		stage.onenterframe = function(){
			...
			if( endless && !this.miss ){
				...
				if( score >= level*levelup ){
					var text = new Text(320,152,"LEVEL UP!");
					text.tl.moveTo(88,152,sec(1),enchant.Easing.QUINT_EASEINOUT).
					delay(sec(1)).
					moveTo(-144,152,sec(1),enchant.Easing.QUINT_EASEINOUT).
					then(function(){stage.removeChild(this);});
					this.addChild(text);
					level++;
					if( level > 5 )levelup = 50000;
					if( level > 10 )levelup = 100000;
					this.speed -= sec(0.2);
					if( this.speed < sec(1.5) )this.speed = sec(1.5);
					if( level == 10 )this.speed = sec(3);	//レベル１０で一旦速度を戻す
				}
...
*/
		...
		scoreLabel.x = 5;
		scoreLabel.y = 0;
		scoreLabel.font = "bold 13px 'Helvetica','Arial','sans-serif'";
//		scoreLabel.textAlign = "center";
		scoreLabel.color = "white";
		scoreLabel.backgroundColor = "rgb(0,0,0)";
...
//		layer_system.addChild(timer);
		...
		life.text.font = "bold 13px 'Helvetica','Arial','sans-serif'";
				...
				if( items[i].within(this.sprite,16) ){
					switch(items[i].type){
						...
						case 6:	//メロン
							stage.getItem++;
							//ポイント表示
							var pt = new Text( this.x-5, this.y, ""+(items[i].point+(level-1)*100) );
							pt.opacity = 0;
							pt.point = items[i].point+(level-1)*100;
						...
						case 3:	//時計
							this.numGetItem++;
							this.tl.delay(30);
							var pt = new Text( this.x-64, this.y, "SPEED DOWN" );
							pt.opacity = 0;
							pt.point = items[i].point;
							pt.tl.fadeIn(1).moveBy(0,-30,30).and().fadeOut(30).delay(60);
							pt.tl.then(function(){
								stage.speed -= sec(0.8);
								stage.removeChild(this);
							});
		...
		var chainCheck = function(panel){
			if( panel.chain > 4 && endless ){
				...
				if( stage.chain == 5 || stage.chain % 10 == 0 ){
					...
					pt.opacity = 0;
					pt.point = 5000*stage.chain/5+((level-1)*1000);
...
/*
		//上下左右ループ用
		var sp = new Sprite(60,60);
		sp.image = game.assets['panel.png'];
		sp.frame = 2;
		sp.x = 160-30;
		sp.y = -45;
		sp.visible = false;
		sp.onenterframe = function(){if(endless)this.visible=true;}
		layer_panel.addChild(sp);
		var sp = new Sprite(60,60);
		sp.image = game.assets['panel.png'];
		sp.frame = 2;
		sp.x = 160-30;
